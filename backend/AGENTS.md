...
- Resume an interrupted full-image run with the same fold, crop-recipe flags, and `--name`, plus `--resume-from runs/<run>/weights/last.pt`; the trainer rematerializes the temporary fold and preserves optimizer, scheduler, and early-stopping state. Keep `early_stopping_state.json` with `weights/last.pt`; resume requires its checkpoint hash/epoch and original patience to match. Missing state blocks resume.
- Full-image runs retain original dataset provenance before the first epoch. Resume verifies the fold, manifest hashes, materialized image/label contents, crop recipe, augmentation, and Ultralytics version; missing or incomplete original provenance blocks resume. Do not reconstruct it from current data.
...
- Sequence, UI shadow, and sensitivity evaluations must verify the requested fold and the exact CV manifest SHA-256 against original checkpoint training provenance before preparing images or starting services. Use `--folds` (Python) or `FULL_IMAGE_DIGIT_SHADOW_CV_FOLDS_PATH` (UI) for the original manifest after dataset changes; never rewrite provenance to match current assignments. The UI derives its fold from provenance and rejects conflicting overrides.
