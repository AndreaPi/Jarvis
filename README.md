...
- The selection layer is edge-first: all `90/270` edge candidates are evaluated first, then a narrow `scan-roi` / base fallback pass runs only when edge evidence is still single-source; low-confidence edge reads can still be rejected at the final gate.
